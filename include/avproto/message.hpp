...
message::message_packet decode_message(const std::string& payload);

// 解码用的 key 是个 base64 编码的字符串. 加密类型和加密密钥都在里面. 这个 key 字符串由管理员在你进群的时候发送过来
message::message_packet decode_message(const std::string& encryption_key, const std::string& payload);
